...
<jsp:include page="../head.jsp">
	<jsp:param value="标签分类信息" name="title" />
</jsp:include>
</head>
	...
	<div class="container-fluid">
				...
				<div class="panel panel-success">
						...
						<form class="form-inline" role="search" method="post"
							...
							<div class="form-group pull-right">
								...
								<a class="btn btn-sm btn-success" data-toggle="tooltip"
									title="添加标签"
									href="${pageContext.request.contextPath }/admin/admin_tag_add"
					...
					<div class="panel-body">
						<table class="table table-hover">
							...
							<c:if test="${findTagList ne null}">
								<c:forEach items="${findTagList}" var="tag"
									varStatus="vs">
									<tr align="center">
										<td>${vs.count }</td>
										<td>${tag.id }</td>
										<td>${tag.tagname }</td>
										<td><a
											href="${pageContext.request.contextPath}/admin/admin_tag_edit?id=${tag.id}"><button
													type="button" class="btn btn-xs btn-info"
													data-toggle="modal" data-target="#delConfirmModal"
													data-toggle="tooltip" title="修改标签">
													<span class="glyphicon glyphicon-edit" aria-hidden="true"></span>
												</button></a>
												<button type="button" class="btn btn-xs btn-warning"
												data-toggle="tooltip" title="删除标签"
												onclick="delConfirm(${tag.id})">
		...
		<script>
					function delConfirm(id) {
						$('#url').val('${pageContext.request.contextPath}/admin/admin_tag_del?id=' + id);
